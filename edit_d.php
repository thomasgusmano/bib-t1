...
if (!empty($_POST)) {
    ...
    if ($operacion == 'edicion' || $operacion == 'ver') {
        $id = $_POST['id_lib'];
        $A = $libro_d->traer_datos($objConexion->getConexion(), $id);
        ...
        <form role="form" method="POST" action="<?php echo $accion; ?>">
          ...
          if (isset($operacion)) {
            if ($operacion == 'edicion' || $operacion == 'baja') {
              echo '<label for="id_usuario">ID:</label>';
              echo '<input id="id_lib" name="id_lib" type="text" class="form-control" disabled value="' . $id . '"/>';
            }
          }
          ?> 
          <div class="row">
